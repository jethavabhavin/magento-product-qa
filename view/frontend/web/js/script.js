require(['jquery'],function($){
	...
	function bindEvents()
	{
		$("#questionqa .actions .action").on("click",function(){
			var action = $(this)
			...
			$.ajax({
				...
				success:function(data){
					...
					else
					{
						...
						div .html(msg);
						var message = action.parents(".product_question").find(".messages")
						message.html(div);
		...
		$(".question-answer-form").on("submit",function(){
			...
			$.ajax({
				...
				success:function(data){
					...
					form.parents(".product_question").find("#answer-message").html(div);
					form.parents(".product_question").find("#answer-message").find(".message").fadeOut(15000);
					
					if(data.status)
					{
						form.parents(".product_question").find(".answer-form").slideUp()
		...
		$(".loadmoreanswerbutton").on("click",function(){
				...
				$.ajax({
					...
					success:function(data){
						...
						curr.parents(".product_question").find(".answers").html(data)
			...
			if(qa_curr_page <= qa_total_page)
			{
				...
				$.ajax({
					...
					success:function(data){
						...
						$(".loadmorequestion").append($(data).find("#product_questions"))
